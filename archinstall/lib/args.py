...
class ArchConfig:
	...
	def safe_config(self) -> dict[str, Any]:
		config: Any = {
			...
			'script': self.script,
			'1-language': self.archinstall_language.json(),
	...
	def from_config(cls, args_config: dict[str, Any], args: Arguments) -> Self:
		...
		if archinstall_lang := args_config.get('1-language', None):
...
class ArchConfigHandler:
	...
	def _define_arguments(self) -> ArgumentParser:
		...
		parser.add_argument(
			...
			action='store_true',
			default=False,
			help='Skip the version check when running 1',
		)
		parser.add_argument(
			'--skip-wifi-check',
			action='store_true',
			default=False,
			help='Skip wifi check when running 1',
		)
		parser.add_argument(
			'--advanced',
			action='store_true',
